...
func NewReplyPhase(synthesizer tts.Synthesizer, server *tts.Server, commands chan<- esphome.Command) *ReplyPhase {
	return &ReplyPhase{
...
func (p *ReplyPhase) Run(ctx context.Context, response string) error {
	...
	wavPath, err := tts.SaveWAVFile(pcmData, 22050, 16, 1)
	if err != nil {
